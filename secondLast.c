...
void test(FusionMatrix gyroscopeMisalignment,
    ...
    // while (true) {
        ...
        fprintf(file, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f", internal.accelerationError, internal.accelerometerIgnored, internal.accelerationRecoveryTrigger, internal.magneticError, internal.magnetometerIgnored, internal.magneticRecoveryTrigger, flags.initialising, flags.angularRateRecovery, flags.accelerationRecovery, flags.magneticRecovery);
        fprintf(file, "\n");
...
int main() {
    ...
    FILE *file = fopen("secondLast6.txt", "a+"); // Open the file for appending or create it if it doesn't exist
    if (!file) {
